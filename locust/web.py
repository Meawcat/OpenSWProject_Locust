...
class WebUI:
  ...
  def __init__(
    ...
    def index() -> str | Response:
      print(argument_parser.Mode)
      if argument_parser.Mode == "nomal":
        print(11111)
      ...
      if argument_parser.Mode == "basic":
        print(22222)
        if not environment.runner:
          return make_response(
            "Error: Locust Environment does not have any runner", 500)
        self.update_template_args()
        return render_template("basic.html", **self.template_args)

      if argument_parser.Mode == "password":
        print(33333)
        if not environment.runner:
          return make_response(
            "Error: Locust Environment does not have any runner", 500)
        self.update_template_args()
        return render_template("password.html", **self.template_args)

      if argument_parser.Mode == "ddos":
        print(44444)
        if not environment.runner:
          return make_response(
            "Error: Locust Environment does not have any runner", 500)
        self.update_template_args()
        return render_template("ddos.html", **self.template_args)
    ...
    def swarm() -> Response:
        ...
        if form_data_user_class_names:
          ...
          if self.environment.runner and self.environment.runner.state == STATE_RUNNING:
            ...
            user_classes = {
              key: value
              for (key,
                   value) in self.environment.available_user_classes.items()
      ...
      parsed_options_dict = vars(
        environment.parsed_options) if environment.parsed_options else {}
      run_time = None
      ...
      if environment.runner is not None:
        ...
        if run_time:
          gevent.spawn_later(
            run_time,
            self._stop_runners).link_exception(greenlet_exception_handler)
    ...
    def exceptions() -> Response:
      return jsonify({
        ...
        } for row in (environment.runner.exceptions.values() if environment.
                      runner is not None else [])]
      })
    ...
    def tasks() -> Dict[str, Dict[str, Dict[str, float]]]:
      ...
      if runner is None:
        ...
        user_spawned = (runner.reported_user_classes_count if isinstance(
          runner, MasterRunner) else runner.user_classes_count)
  ...
  def update_template_args(self):
    ...
    available_user_classes = (None if
                              not self.environment.available_user_classes else
                              sorted(self.environment.available_user_classes))
