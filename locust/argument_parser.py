...
def find_locustfiles(locustfiles: List[str], is_directory: bool) -> List[str]:
  ...
  if is_directory:
    ...
    if len(locustfiles) != 1:
      sys.stderr.write(
        f"Multiple values passed in for directory: {locustfiles}\n")
      sys.exit(1)
    ...
    for root, dirs, files in os.walk(locustdir):
      for file in files:
        if not file.startswith(
            "_") and file.lower() != "locust.py" and file.endswith(".py"):
    ...
    for file_path in locustfiles:
      if not file_path.endswith(".py"):
        sys.stderr.write(
          f"Invalid file '{file_path}'. File should have '.py' extension\n")
        sys.exit(1)
      if file_path.endswith("locust.py"):
        sys.stderr.write(
          "Invalid file 'locust.py'. File name cannot be 'locust.py'\n")
        sys.exit(1)
...
def get_empty_argument_parser(
    add_help=True,
    default_config_files=DEFAULT_CONFIG_FILES) -> LocustArgumentParser:
  ...
  parser.add_argument(
    ...
    default="locustfile",
    help=
    "Python module to import, e.g. '../other_test.py'. Either a .py file, multiple comma-separated .py files or a package "
...
def parse_locustfile_option(args=None) -> List[str]:
  ...
  """
    ...
    Returns:
        Locustfiles (List): List of locustfile pathss
    """
  ...
  locustfile_as_list = [
    locustfile.strip() for locustfile in options.locustfile.split(",")
  ]

  print(locustfile_as_list)
  #  basic  으로 바로 넘기기
  if "basic" in locustfile_as_list:
    print("Run Basic !!!!!")
    Mode = "basic"
    locustfiles = [inspect.getfile(locust)[0:-12] + '/basic.py']
    return locustfiles

  #  password  으로 바로 넘기기
  if "password" in locustfile_as_list:
    print("Run password !!!!!")
    Mode = "password"
    locustfiles = [inspect.getfile(locust)[0:-12] + '/password.py']
    return locustfiles

  #  ddos  으로 바로 넘기기
  if "ddos" in locustfile_as_list:
    print("Run ddos !!!!!")
    Mode = "ddos"
    locustfiles = [inspect.getfile(locust)[0:-12] + '/ddos.py']
    return locustfiles
...
def setup_parser_arguments(parser):
  ...
  web_ui_group.add_argument(
    ...
    action="store_true",
    help=
    "Starts the test immediately (without disabling the web UI). Use -u and -t to control user count and run time",
...
Only the LOCUSTFILE (-f option) needs to be specified when starting a Worker, since other options such as -u, -r, -t are specified on the Master node.""",
  ...
  step_load_group.add_argument("--step-load",
                               action="store_true",
                               help=configargparse.SUPPRESS)
  step_load_group.add_argument("--step-users",
                               type=int,
                               help=configargparse.SUPPRESS)
  step_load_group.add_argument("--step-clients",
                               action="store_true",
                               help=configargparse.SUPPRESS)
  ...
  other_group.add_argument(
    ...
    version=
    f"locust {version} from {os.path.dirname(__file__)} (python {platform.python_version()})",
  )
  other_group.add_argument(
    "--exit-code-on-error",
    type=int,
  ...
  user_classes_group.add_argument(
    ...
    metavar="UserClass",
    help=
    "Optionally specify which User classes that should be used (available User classes can be listed with -l or --list)",
  )
